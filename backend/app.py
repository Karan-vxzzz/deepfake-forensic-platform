...
if st.session_state.scan_done and st.session_state.results:
    ...
    scan_data = {
        "scan_id": verdict.get("scan_id", "N/A"),
        "timestamp": verdict.get("timestamp", "N/A"),
        "is_fake": verdict.get("is_fake", False),
        "verdict_title": verdict.get("verdict_title") or v_label.replace("⚠ ", "").replace("✔ ", ""),
        "confidence": verdict.get("confidence", 0.0),
        "threat_level": verdict.get("threat_level") or verdict.get("risk_level", ""),
        "risk_level": verdict.get("risk_level", ""),
        "filename": st.session_state.metadata.get("filename", "Unknown"),
        "media_type": mtype,
        "engine_results": res.get("engine_results") or res.get("engines", {}),
        "img_b64": res.get("img_b64", {}).copy() if mtype == "image" else {}
    }

    if mtype == "video":
        scan_data["sus_frames"] = [to_b64(fr) for fr in res.get("sus_frames", [])]
        er = res["engine_results"]
        timelines = [er[k].get("timeline",[]) for k in ["temporal","identity","gan","micro_expr","blink","texture","av_sync"] if er[k].get("timeline",[])]
        if timelines:
            min_len = min(len(t) for t in timelines)
            ...
            scan_data["img_b64"]["timeline"] = _fig_to_b64(fig)
        ...
        if spec:
            ...
            fig.patch.set_facecolor('#060b14')
            ax.axis('off')
            scan_data["img_b64"]["spectrogram"] = _fig_to_b64(fig)
        if len(mfcc) > 0:
            fig, ax = plt.subplots(figsize=(6, 3))
            ...
            fig.patch.set_facecolor('#060b14')
            ax.axis('off')
            scan_data["img_b64"]["mfcc"] = _fig_to_b64(fig)

    try:
        pdf_data = pdf_generator.generate_enterprise_pdf(scan_data)
        import datetime
        now_str = datetime.datetime.now().strftime("%Y_%m_%d_%H_%M")
        
        st.download_button(
            label="⬇ DOWNLOAD PROFESSIONAL FORENSIC REPORT",
            data=pdf_data,
            file_name=f"FORENSIC_REPORT_{now_str}.pdf",
